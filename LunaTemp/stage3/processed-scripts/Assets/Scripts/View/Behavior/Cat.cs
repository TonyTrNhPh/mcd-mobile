...
public class Cat : MonoBehaviour, IPointerDownHandler, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        if (targetSlot.IsEmpty)
        {
            ...
            if (targetSlot.Type == SlotType.Box)
            {
                GameEvent.HandleCatRegistered(targetSlot);
            }
        }
        else
        {
    ...
    private void Shoot()
    {
        ...
        Vector2 direction = target.transform.position - firePoint.transform.position;
        projectile.Initialize(direction.normalized, CalculateDamage(_mergeLevel, _upgradeLevel));
    }
